...
import { streamLinkedInPost } from '@/lib/claude';
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        console.error('[api/repurpose/linkedin-post/stream] Error:', err);
        controller.enqueue(
          encoder.encode(
            sseEvent('error', {
              type: 'error',
              message: 'Failed to generate LinkedIn post. Please try again.',
