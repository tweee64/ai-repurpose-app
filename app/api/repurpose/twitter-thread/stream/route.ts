...
import { streamTwitterThread } from '@/lib/claude';
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        await streamTwitterThread(
          transcript.text,
          (tweet) => {
            collectedTweets.push(tweet);
            controller.enqueue(
              encoder.encode(
                sseEvent('tweet', { type: 'tweet', index: tweet.index, text: tweet.text }),
              ),
            );
          },
        ...
        // Persist draft after all tweets have been collected
        const draft = await prisma.draft.create({
          data: {
            userId,
            transcriptId: transcript.id,
            format: 'twitter_thread',
            content: JSON.stringify({ tweets: collectedTweets }),
          },
        });
        ...
        console.error('[api/repurpose/twitter-thread/stream] Error:', err);
        controller.enqueue(
          encoder.encode(
            sseEvent('error', {
              type: 'error',
              message: 'Failed to generate Twitter thread. Please try again.',
